...
export function mnistDemo(divId, canvasId) {
  ...
  async function loadMnistSamples() {
    ...
    const mnistBmp = await toBmp("/static/image_data/mnist.png");  //await (await fetch("mnist.png")).blob());
  ...
  const run = async () => {
      await initUI();
      const r = await fetch("/static/model.json");
      const consts = parseConsts(await r.json());
      const model = await tf.loadGraphModel("/static/model.json");
